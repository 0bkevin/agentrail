...
export async function POST(request: Request) {
  try {
    ...
    if (order.serviceType !== "human_task") {
      return Response.json(
        { ok: false, error: `Order ${order.id} is not a human-task fulfillment.` },
        { status: 400 },
      );
    }
    ...
    if (order.providerWallet && order.providerWallet.toLowerCase() !== account.address.toLowerCase()) {
      return Response.json(
        { ok: false, error: "Signer address does not match assigned human provider wallet." },
        { status: 400 },
      );
    }
    ...
    return Response.json(
      { ok: false, error: error instanceof Error ? error.message : "Failed to generate human proof." },
      { status: 500 },
    );
  }
}
